 ...
 %% Load coded data
        ...
        correvents = {'S150' 'S160'};
            ...
            for j = 1:length(EEG.event)
                if strcmp(EEG.event(j).type, 'S 16')
                    idxStart = j;
                elseif strcmp(EEG.event(j).type, 'S111')
                    idxEnd = j;
                end
            end
            ...
            if i == 27
                for j = 1:length(EEG.event)
                    if strcmp(EEG.event(j).type, 'S117')
                        idxEnd = j;
                    end
                end
            end
...
for m = 1: length(EEG.event)
    if strcmp(EEG.event(m).type,'S 71') || strcmp(EEG.event(m).type,'S 72') 
a = a + 1;
    end
...
%%  preprocess EEG data  
       ...
       % Resample
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_EEG_' num2str(i) '_Sternberg.csv' ],  rejected_channels)
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_ICA_' num2str(i) '_Sternberg.csv' ],  rejected_channels)
       ...
       % Interpolate data channels
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_' num2str(i) '_Sternberg.csv' ],  rejected_segments_EEG)
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsICA_' num2str(i) '_Sternberg.csv' ],  rejected_segments_ICA)
      ...
      %  ICA = iclabel(ICA, 'default');
       ...
       csvwrite([PATH_BADICS 'BadICs_' num2str(i) '_Sternberg.csv' ],  EEG.ICout_IClabel)
       ...
       [EEG, segs_rej] = pop_autorej(EEG, 'nogui', 'on', 'threshold', 1000, 'startprob', 5, 'maxrej', 5);
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_after_' num2str(i) '_Sternberg.csv' ],  segs_rej)
          
        
        % Save autocleaned data
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_cleaned_Sternberg.set'], 'filepath', PATH_AUTOCLEANED, 'check', 'on', 'savemode', 'twofiles');
...
csvwrite([PATH_MAIN 'Not_existing_files_Sternberg.csv' ],  NotExistingFile)
save([PATH_MAIN 'Interindividuell_ACC_outliers_Sternberg.mat' ],  'ACCOutliers')
...
ACC_Out = load([PATH_MAIN 'Interindividuell_ACC_outliers_Sternberg.mat' ]);
    
        for i=1:nSubjects
            % Load data
            subject = num2str(i);
            datafile = [num2str(i) '_cleaned_Sternberg.set'];
            
            
            if isfile([PATH_AUTOCLEANED datafile]) == 1 && ~ismember(i, ACC_Out.ACCOutliers)
                EEG = pop_loadset('filename', [subject '_cleaned_Sternberg.set'], 'filepath', PATH_AUTOCLEANED, 'loadmode', 'all');
               ...
               %odd even split for reliability
                ...
                EEG.setname = ['Sternberg_' num2str(i) '_bin'];
                
                % Epoch data
                EEG = pop_epochbin( EEG , [-200.0  1000.0],  [ -200 0]);
                EEG.setname = ['Sternberg_' num2str(i) '_bin_epoch'];
                
                % Compute ERPs for the different bins
                ERP = pop_averager( EEG , 'Criterion', 'all', 'ExcludeBoundary', 'off', 'SEM', 'on' );
                ERP.erpname = ['Sternberg_' num2str(i) '_bin_average'];
               
           
                % Save ERP set to
                ERP = pop_savemyerp(ERP, 'erpname', ERP.erpname, 'filename', ['Sternberg_' num2str(i) '.erp'], 'filepath', PATH_ERP);
            else
         ...
         csvwrite([PATH_MAIN 'Sternberg_count_trials_in_bnis.csv'], count_trials)
