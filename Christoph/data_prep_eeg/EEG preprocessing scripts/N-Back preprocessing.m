...
for i = 1 : nSubjects
    ...
    elseif i == 43
        datafile = ['Keine Aufzeichnung'];
    elseif i == 119
        datafile = ['Exp23_0' num2str(i) '1.vhdr'];
 ...
 %% Load coded data
        ...
        for j = 1:length(EEG.event)
            if strcmp(EEG.event(j).type, 'S  8')
                idxStart = j;
            elseif strcmp(EEG.event(j).type, 'S113')
                idxEnd = j;
            end
        end
        ...
        EEG.event = EEG.event(idxStart:idxEnd); % Anfang und Ende sind nun definiert
        
        for j = 1:length(EEG.event)
            if strcmp(EEG.event(j).type, 'S102')
                idxStart = j;
            elseif strcmp(EEG.event(j).type, 'S203')
                idxEnd = j;
            end
        end
            ...
            if ismember(EEG.event(stimidx(e)+1).type, correvents) && ... % ist die Resp korrekt
                ...
                if strcmp(EEG.event(stimidx(e)).type,'S 53') ||  strcmp(EEG.event(stimidx(e)).type,'S 54')
                    if strcmp(EEG.event(stimidx(e)).type,'S 53')
                        EEG.event(stimidx(e)).Match = 'Match';
                        EEG.event(stimidx(e)).MatchCond = 1;
                    else
                        EEG.event(stimidx(e)).Match = 'NoMatch';
                        EEG.event(stimidx(e)).MatchCond = 2;
                    end
                    EEG.event(stimidx(e)).Cond_Factor = 1;
                    EEG.event(stimidx(e)).Condition = 'N-Back_1Back';
                elseif strcmp(EEG.event(stimidx(e)).type,'S 55') ||  strcmp(EEG.event(stimidx(e)).type,'S 56')
                    ...
                    EEG.event(stimidx(e)).Condition = 'N-Back_2Back';
                end
...
for m = 1: length(EEG.event)
    if strcmp(EEG.event(m).type,'S 43') || strcmp(EEG.event(m).type,'S 44') || strcmp(EEG.event(m).type,'S 45') || strcmp(EEG.event(m).type,'S 46') 
a = a + 1;
    end
...
for m = 1: length(EEG.event)    
    if strcmp(EEG.event(m).type,'stim') 
        EEG.event(m).z_RT_overall = z_RT3(c);
        c = c + 1;
    end    
end
...
%%  preprocess EEG data  
       ...
       % Resample
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_EEG_' num2str(i) '_N_Back_Task.csv' ],  rejected_channels)
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_ICA_' num2str(i) '_N_Back_Task.csv' ],  rejected_channels)
       ...
       % Interpolate data channels
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_' num2str(i) '_N_Back_Task.csv' ],  rejected_segments_EEG)
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsICA_' num2str(i) '_N_Back_Task.csv' ],  rejected_segments_ICA)
        ...
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_N_Back_Task_icdata.set'], 'filepath', PATH_ICWEIGHTS, 'check', 'on', 'savemode', 'twofiles');
       ...
       csvwrite([PATH_BADICS 'BadICs_' num2str(i) '_N_Back_Task.csv' ],  EEG.ICout_IClabel)
       ...
       [EEG, segs_rej] = pop_autorej(EEG, 'nogui', 'on', 'threshold', 1000, 'startprob', 5, 'maxrej', 5);
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_after_' num2str(i) '_N_Back_Task.csv' ],  segs_rej)
                     
        % Save autocleaned data
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_cleaned_N_Back_Task.set'], 'filepath', PATH_AUTOCLEANED, 'check', 'on', 'savemode', 'twofiles');
...
csvwrite([PATH_MAIN 'Not_existing_files_N_Back_Task.csv' ],  NotExistingFile)
save([PATH_MAIN 'Interindividuell_ACC_outliers_N_Back_Task.mat' ],  'ACCOutliers')
...
%% ERP 
    ...
    count_trials = [];
     ...
     ACC_Out = load([PATH_MAIN 'Interindividuell_ACC_outliers_N_Back_Task.mat']);
    
        for i=1:nSubjects
            % Load data
            subject = num2str(i);
            datafile = [num2str(i) '_cleaned_N_Back_Task.set'];
            
            
            if isfile([PATH_AUTOCLEANED datafile]) == 1 && ~ismember(i, ACC_Out.ACCOutliers)
                EEG = pop_loadset('filename', [subject '_cleaned_N_Back_Task.set'], 'filepath', PATH_AUTOCLEANED, 'loadmode', 'all');
                
                % Recode event types for ERPlab
                for j = 1:length(EEG.event)
                    if strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'N-Back_1Back') && ...
                         strcmp(EEG.event(j).Match, 'Match')
                        EEG.event(j).type = 1;
                    elseif strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'N-Back_1Back')&& ...
                         strcmp(EEG.event(j).Match, 'NoMatch')
                        EEG.event(j).type = 2;
                    elseif strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'N-Back_2Back') && ...
                         strcmp(EEG.event(j).Match, 'Match')
                        EEG.event(j).type = 3;
                    elseif strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'N-Back_2Back') && ...
                         strcmp(EEG.event(j).Match, 'NoMatch')
               ...
               for j = 1:length(EEG.event)
                   if isempty(EEG.event(j).Odd_Even) == 0
                       if EEG.event(j).Odd_Even == 2 &&  EEG.event(j).type == 1
                           EEG.event(j).type = 11;
                       elseif  EEG.event(j).Odd_Even == 2 &&  EEG.event(j).type == 2
                           EEG.event(j).type = 22;
                       elseif  EEG.event(j).Odd_Even == 2 &&  EEG.event(j).type == 3
                           EEG.event(j).type = 33;
                       elseif  EEG.event(j).Odd_Even == 2 &&  EEG.event(j).type == 4
                           EEG.event(j).type = 44;
                       end
                   end
               end
                ...
                for t = 1:length(EEG.event)
                    if isempty(EEG.event(t).Odd_Even) == 0
                        ...
                        elseif   EEG.event(t).type == 11
                            b = b + 1;
                        elseif   EEG.event(t).type == 2
                            c = c + 1;
                        elseif   EEG.event(t).type == 22
                            d = d + 1;
                        elseif  EEG.event(t).type == 3
                            e = e + 1;
                        elseif   EEG.event(t).type == 33
                            f = f + 1;
                        elseif   EEG.event(t).type == 4
                            g = g + 1;
                ...
                EEG  = pop_binlister(EEG , 'BDF', [PATH_LIST '\bin_list_Exp23_N_Back_task.txt'], 'IndexEL',  1, 'SendEL2', 'Workspace&EEG', 'UpdateEEG', 'on', 'Voutput', 'EEG' );
                EEG.setname = ['N_Back_Task_' num2str(i) '_bin'];
                
                % Epoch data
                EEG = pop_epochbin( EEG , [-200.0  1000.0],  [ -200 0]);
                EEG.setname = ['N_Back_Task_' num2str(i) '_bin_epoch'];
                
                % Compute ERPs for the different bins
                ERP = pop_averager( EEG , 'Criterion', 'all', 'ExcludeBoundary', 'off', 'SEM', 'on' );
                ERP.erpname = ['N_Back_Task_' num2str(i) '_bin_average'];
                
                % Save ERP set to
                ERP = pop_savemyerp(ERP, 'erpname', ERP.erpname, 'filename', ['N_Back_Task_' num2str(i) '.erp'], 'filepath', PATH_ERP);
            else
        ...
        csvwrite([PATH_MAIN 'N_Back_Task_count_trials_in_bnis.csv'], count_trials)
