...
for i = 1 : nSubjects
    ...
    elseif i == 6
        datafile = ['Exp23_000' num2str(i) '_2_2.vhdr'];
    elseif i == 52
        datafile = ['Exp23_00' num2str(i) '_21.vhdr'];
    elseif i == 138
        datafile = ['Exp23_0' num2str(i) '_21.vhdr'];
    elseif i == 50 || i == 63 || i == 73 || i == 80 || i == 88 || i == 97
 ...
 %% Load coded data
        ...
        for j = 1:length(EEG.event)
            if strcmp(EEG.event(j).type, 'S 18')
                idxStart = j;
            elseif strcmp(EEG.event(j).type, 'S 19')
                idxEnd = j;
            end
        end
        ...
        for e = 1 : length(stimidx)
            if ismember(EEG.event(stimidx(e)+1).type, correvents) && ... %!!!!!!!!!!
                ...
                if strcmp(EEG.event(stimidx(e)).type,'S 51')
                    EEG.event(stimidx(e)).Condition = 'NoPriming';
                    EEG.event(stimidx(e)).Cond_Factor = 1;
                elseif strcmp(EEG.event(stimidx(e)).type,'S 52')
                    EEG.event(stimidx(e)).Condition = 'Priming';
...
%%  preprocess EEG data  
       ...
       % Resample
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_EEG_' num2str(i) '_NegativePriming.csv' ],  rejected_channels)
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_ICA_' num2str(i) '_NegativePriming.csv' ],  rejected_channels)
       ...
       % Interpolate data channels
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_' num2str(i) '_NegativePriming.csv' ],  rejected_segments_EEG)
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsICA_' num2str(i) '_NegativePriming.csv' ],  rejected_segments_ICA)
        ...
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_NegativePriming_icdata.set'], 'filepath', PATH_ICWEIGHTS, 'check', 'on', 'savemode', 'twofiles');
       ...
       csvwrite([PATH_BADICS 'BadICs_' num2str(i) '_NegativePriming.csv' ],  EEG.ICout_IClabel)
       ...
       [EEG, segs_rej] = pop_autorej(EEG, 'nogui', 'on', 'threshold', 1000, 'startprob', 5, 'maxrej', 5);
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_after_' num2str(i) '_NegativePriming.csv' ],  segs_rej)
               
        % Save autocleaned data
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_cleaned_NegativePriming.set'], 'filepath', PATH_AUTOCLEANED, 'check', 'on', 'savemode', 'twofiles');
...
csvwrite([PATH_MAIN 'Not_existing_files_NegativePriming.csv' ],  NotExistingFile)
save([PATH_MAIN 'Interindividuell_ACC_outliers_NegativePriming.mat' ],  'ACCOutliers')
...
%% ERP 
    ...
    count_trials = [];
     ...
     ACC_Out = load([PATH_MAIN 'Interindividuell_ACC_outliers_NegativePriming.mat' ]);


        for i=1:nSubjects
            % Load data
            subject = num2str(i);
            datafile = [num2str(i) '_cleaned_NegativePriming.set'];
            
            
            if isfile([PATH_AUTOCLEANED datafile]) == 1 && ~ismember(i, ACC_Out.ACCOutliers)
                EEG = pop_loadset('filename', [subject '_cleaned_NegativePriming.set'], 'filepath', PATH_AUTOCLEANED, 'loadmode', 'all');
                
                % Recode event types for ERPlab
                for j = 1:length(EEG.event)
                    if strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'NoPriming')
                        EEG.event(j).type = 1;
                    elseif strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'Priming')
                        EEG.event(j).type = 2;
                    end
                end
               ...
               %odd even split for reliability
                ...
                EEG  = pop_binlister(EEG , 'BDF', [PATH_LIST '\bin_list_Exp23_inhibition_tasks.txt'], 'IndexEL',  1, 'SendEL2', 'Workspace&EEG', 'UpdateEEG', 'on', 'Voutput', 'EEG' );
                EEG.setname = ['NegativePriming_' num2str(i) '_bin'];
                
                % Epoch data
                EEG = pop_epochbin( EEG , [-200.0  1000.0],  [ -200 0]);
                EEG.setname = ['NegativePriming_' num2str(i) '_bin_epoch'];
                
                % Compute ERPs for the different bins
 
                ERP = pop_averager( EEG , 'Criterion', 'all', 'ExcludeBoundary', 'off', 'SEM', 'on' );
                ERP.erpname = ['NegativePriming_' num2str(i) '_bin_average'];
                
                % Save ERP set to
                ERP = pop_savemyerp(ERP, 'erpname', ERP.erpname, 'filename', ['NegativePriming_' num2str(i) '.erp'], 'filepath', PATH_ERP);
            else
          ...
          csvwrite([PATH_MAIN 'NegativePriming_count_trials_in_bnis.csv'], count_trials)
