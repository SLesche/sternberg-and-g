 ...
 %% Load coded data
        ...
        correvents = {'S150' 'S160'};
            ...
            for j = 1:length(EEG.event)
                if strcmp(EEG.event(j).type, 'S  4')
                    idxStart = j;
                elseif strcmp(EEG.event(j).type, 'S116')
                    idxEnd = j;
                end
            end
        ...
        for e = 1 : length(stimidx)
            if ismember(EEG.event(stimidx(e)+1).type, correvents) && ... %!!!!!!!!!!
                ...
                if strcmp(EEG.event(stimidx(e)).type,'S 54')
                    EEG.event(stimidx(e)).Condition = 'left';
                    EEG.event(stimidx(e)).Cond_Factor = 1;
                elseif strcmp(EEG.event(stimidx(e)).type,'S 55')
                    EEG.event(stimidx(e)).Condition = 'right';
...
for m = 1: length(EEG.event)
    if strcmp(EEG.event(m).type,'S 94') || strcmp(EEG.event(m).type,'S 95') 
a = a + 1;
    end
...
%%  preprocess EEG data  
       ...
       % Resample
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_EEG_' num2str(i) '_ChoiceRT.csv' ],  rejected_channels)
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_ICA_' num2str(i) '_ChoiceRT.csv' ],  rejected_channels)
       ...
       % Interpolate data channels
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_' num2str(i) '_ChoiceRT.csv' ],  rejected_segments_EEG)
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsICA_' num2str(i) '_ChoiceRT.csv' ],  rejected_segments_ICA)
        ...
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_ChoiceRT_icdata.set'], 'filepath', PATH_ICWEIGHTS, 'check', 'on', 'savemode', 'twofiles');
       ...
       csvwrite([PATH_BADICS 'BadICs_' num2str(i) '_ChoiceRT.csv' ],  EEG.ICout_IClabel)
       ...
       [EEG, segs_rej] = pop_autorej(EEG, 'nogui', 'on', 'threshold', 1000, 'startprob', 5, 'maxrej', 5);
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_after_' num2str(i) '_ChoiceRT.csv' ],  segs_rej)
              
        % Save autocleaned data
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_cleaned_ChoiceRT.set'], 'filepath', PATH_AUTOCLEANED, 'check', 'on', 'savemode', 'twofiles');
...
csvwrite([PATH_MAIN 'Not_existing_files_ChoiceRT.csv' ],  NotExistingFile)
save([PATH_MAIN 'Interindividuell_ACC_outliers_ChoiceRT.mat' ],  'ACCOutliers')
...
%% ERP 
   ...
   ACC_Out = load([PATH_MAIN 'Interindividuell_ACC_outliers_ChoiceRT.mat']);
    
    for i=1:nSubjects
        % Load data
        subject = num2str(i);
        datafile = [num2str(i) '_cleaned_ChoiceRT.set'];


        if isfile([PATH_AUTOCLEANED datafile]) == 1 && ~ismember(i, ACC_Out.ACCOutliers)
            EEG = pop_loadset('filename', [subject '_cleaned_ChoiceRT.set'], 'filepath', PATH_AUTOCLEANED, 'loadmode', 'all');
            ...
            %odd even split für Reliabilität
            for j = 1:length(EEG.event)
            ...
            EEG.setname = ['ChoiceRT_' num2str(i) '_bin'];

            % Epoch data
            EEG = pop_epochbin( EEG , [-200.0  1000.0],  [ -200 0]);
            EEG.setname = ['ChoiceRT_' num2str(i) '_bin_epoch'];

            % Compute ERPs for the different bins
            ERP = pop_averager( EEG , 'Criterion', 'all', 'ExcludeBoundary', 'off', 'SEM', 'on' );
            ERP.erpname = ['ChoiceRT_' num2str(i) '_bin_average'];

            % Save ERP set to
            ERP = pop_savemyerp(ERP, 'erpname', ERP.erpname, 'filename', ['ChoiceRT_' num2str(i) '.erp'], 'filepath', PATH_ERP);
    ...
    csvwrite([PATH_MAIN 'ChoiceRT_count_trials_in_bnis.csv'], count_trials)
