 ...
 %% Load coded data
        ...
        for j = 1:length(EEG.event)
            if strcmp(EEG.event(j).type, 'S  2')
                idxStart = j;
            elseif strcmp(EEG.event(j).type, 'S117')
                idxEnd = j;
            end
        end
...
for m = 1: length(EEG.event)
    if strcmp(EEG.event(m).type,'S 91') || strcmp(EEG.event(m).type,'S 92')
a = a + 1;
    end
...
%%  preprocess EEG data  
       ...
       % Resample
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_EEG_' num2str(i) '_Flanker.csv' ],  rejected_channels)
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_ICA_' num2str(i) '_Flanker.csv' ],  rejected_channels)
       ...
       % Interpolate data channels
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_' num2str(i) '_Flanker.csv' ],  rejected_segments_EEG)
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsICA_' num2str(i) '_Flanker.csv' ],  rejected_segments_ICA)
        ...
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_Flanker_icdata.set'], 'filepath', PATH_ICWEIGHTS, 'check', 'on', 'savemode', 'twofiles');
       ...
       csvwrite([PATH_BADICS 'BadICs_' num2str(i) '_Flanker.csv' ],  EEG.ICout_IClabel)
       ...
       [EEG, segs_rej] = pop_autorej(EEG, 'nogui', 'on', 'threshold', 1000, 'startprob', 5, 'maxrej', 5);
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_after_' num2str(i) '_Flanker.csv' ],  segs_rej)
              
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_cleaned_Flanker.set'], 'filepath', PATH_AUTOCLEANED, 'check', 'on', 'savemode', 'twofiles');
...
csvwrite([PATH_MAIN 'Not_existing_files_Flanker.csv' ],  NotExistingFile)
save([PATH_MAIN 'Interindividuell_ACC_outliers_Flanker.mat' ],  'ACCOutliers')
...
%% ERP 
    ...
    count_trials = [];
     ...
     ACC_Out = load([PATH_MAIN 'Interindividuell_ACC_outliers_Flanker.mat' ]);
    
        for i=1:nSubjects
            % Load data
            subject = num2str(i);
            datafile = [num2str(i) '_cleaned_Flanker.set'];
            
            
            if isfile([PATH_AUTOCLEANED datafile]) == 1 && ~ismember(i, ACC_Out.ACCOutliers)
                EEG = pop_loadset('filename', [subject '_cleaned_Flanker.set'], 'filepath', PATH_AUTOCLEANED, 'loadmode', 'all');
             ...
             csvwrite([PATH_MAIN 'Flanker_count_trials_in_bnis.csv'], count_trials)
