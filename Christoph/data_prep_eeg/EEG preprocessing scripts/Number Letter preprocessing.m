...
for i = 1 : nSubjects
 ...
 %% Load coded data
        ...
        for j = 1:length(EEG.event)
            if strcmp(EEG.event(j).type, 'S 12')
                idxStart = j;
            elseif strcmp(EEG.event(j).type, 'S119')
                idxEnd = j;
            end
        end
        ...
        if i == 122 || i == 88 || i == 138
            for j = 1:length(EEG.event)
                if strcmp(EEG.event(j).type, 'S 12')
                    idxStart = j;
                elseif strcmp(EEG.event(j).type, 'S 13')
                    idxEnd = j;
                end
            end
        end
...
%%  preprocess EEG data  
       ...
       % Resample
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_EEG_' num2str(i) '_NumberLetter_Task.csv' ],  rejected_channels)
        ...
        csvwrite([PATH_CHANREJECT 'BadChannels_ICA_' num2str(i) '_NumberLetter_Task.csv' ],  rejected_channels)
       ...
       % Interpolate data channels
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_' num2str(i) '_NumberLetter_Task.csv' ],  rejected_segments_EEG)
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsICA_' num2str(i) '_NumberLetter_Task.csv' ],  rejected_segments_ICA)
        ...
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_NumberLetter_Task_icdata.set'], 'filepath', PATH_ICWEIGHTS, 'check', 'on', 'savemode', 'twofiles');
       ...
       csvwrite([PATH_BADICS 'BadICs_' num2str(i) '_NumberLetter_Task.csv' ],  EEG.ICout_IClabel)
       ...
       [EEG, segs_rej] = pop_autorej(EEG, 'nogui', 'on', 'threshold', 1000, 'startprob', 5, 'maxrej', 5);
        ...
        csvwrite([PATH_EPOCHREJECT 'BadSegmentsEEG_after_' num2str(i) '_NumberLetter_Task.csv' ],  segs_rej)
                
        % Save autocleaned data
        EEG = pop_saveset(EEG, 'filename', [num2str(subject) '_cleaned_NumberLetter_Task.set'], 'filepath', PATH_AUTOCLEANED, 'check', 'on', 'savemode', 'twofiles');
...
csvwrite([PATH_MAIN 'Not_existing_files_NumberLetter_Task.csv' ],  NotExistingFile)
save([PATH_MAIN 'Interindividuell_ACC_outliers_NumberLetter_Task.mat' ],  'ACCOutliers')
...
%% ERP 
    ...
    ACC_Out = load([PATH_MAIN 'Interindividuell_ACC_outliers_NumberLetter.mat' ]);
    
        for i=1:nSubjects
            % Load data
            subject = num2str(i);
            datafile = [num2str(i) '_cleaned_NumberLetter_Task.set'];
            
            
            if isfile([PATH_AUTOCLEANED datafile]) == 1  && ~ismember(i, ACC_Out.ACCOutliers)
                EEG = pop_loadset('filename', [subject '_cleaned_NumberLetter_Task.set'], 'filepath', PATH_AUTOCLEANED, 'loadmode', 'all');
                
                % Recode event types for ERPlab
                for j = 1:length(EEG.event)
                    if strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'LessMore_Repeat')
                        EEG.event(j).type1 = 1;
                    elseif strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'LessMore_Switch')
                        EEG.event(j).type1 = 2;
                    elseif strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'VowelConsonant_Repeat')
                        EEG.event(j).type1 = 3;
                    elseif strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'VowelConsonant_Switch')
                        EEG.event(j).type1 = 4;
                    end
                    if strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'LessMore_Repeat') ||...
                            strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'VowelConsonant_Repeat')
                        EEG.event(j).type = 5;
                    elseif strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'LessMore_Switch') ||...
                            strcmp(EEG.event(j).type, 'stim') && strcmp(EEG.event(j).Condition, 'VowelConsonant_Switch')
                        EEG.event(j).type = 6;
                    end   
                end
               ...
               %odd even split for reliability
                ...
                EEG.setname = ['NumberLetter_Task_' num2str(i) '_bin'];
                
                % Epoch data
                EEG = pop_epochbin( EEG , [-200.0  1000.0],  [ -200 0]);
                EEG.setname = ['NumberLetter_Task_' num2str(i) '_bin_epoch'];
                
                % Compute ERPs for the different bins
                ERP = pop_averager( EEG , 'Criterion', 'all', 'ExcludeBoundary', 'off', 'SEM', 'on' );
                ERP.erpname = ['NumberLetter_Task_' num2str(i) '_bin_average'];

                % Save ERP set to
                ERP = pop_savemyerp(ERP, 'erpname', ERP.erpname, 'filename', ['NumberLetter_Task_' num2str(i) '.erp'], 'filepath', PATH_ERP);
            else
         ...
         csvwrite([PATH_MAIN 'NumberLetter_Task_count_trials_in_bnis.csv'], count_trials)