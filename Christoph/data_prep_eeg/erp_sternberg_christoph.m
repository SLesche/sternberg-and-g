...
subject_ids = getIdsFromFolder(PATH_AUTOCLEANED,  'autocleaned_christoph\.fdt$', '(\d+)_autocleaned_christoph\.fdt$');
...
parfor isubject = 1:length(subject_ids)
    ...
    datafile = [subject '_autocleaned_christoph.set'];
    ...
    if isfile([PATH_AUTOCLEANED datafile]) == 1
        ...
        EEG  = pop_binlister(EEG , 'BDF', ['C:\Users\Sven\Documents\projects\research\promotion_mental_speed\sternberg-and-g\Christoph\raw_data_eeg\mzp_1\binlister_stims.txt'], 'IndexEL',  1, 'SendEL2', 'Workspace&EEG', 'UpdateEEG', 'on', 'Voutput', 'EEG' );
        EEG.setname = 'bin';
        ...
        % Save ERP set to
        ERP = pop_savemyerp(ERP, 'erpname', ERP.erpname, 'filename', [num2str(subject_ids(isubject)) '_erp_christoph.erp'], 'filepath', PATH_ERP_AV);
