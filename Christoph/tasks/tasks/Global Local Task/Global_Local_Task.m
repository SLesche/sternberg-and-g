...
% Define a task name
TaskName = 'Shifting Task - Global Local';
...
isPractice = 0;
expinfo = getMarkers(expinfo, isPractice); % load markers
...
expinfo = getMarkers(expinfo, isPractice); % load markers for practice
  ...
  %  io64(expinfo.ioObj, expinfo.PortAddress,0);% Stop Writing to Output Port    
    ...
    % trials
    PracGlob=[expinfo.InstFolder 'PracGlob.jpg'];
    ima=imread(PracGlob, 'jpg');
    ...
    PracLoc=[expinfo.InstFolder 'PracLoc.jpg'];
    ima=imread(PracLoc, 'jpg');
    dImageWait(expinfo,ima);
...
expinfo = getMarkers(expinfo, isPractice); % load markers for exp
...
for expBlock = 1: expinfo.Block
   ...
   % io64(expinfo.ioObj, expinfo.PortAddress,0);% Stop Writing to Output Port
    ...
    BackUp_Trial     = [expinfo.DataFolder,'Backup\',expinfo.taskName,'GlobalLocal_Trials_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session),'_Block',num2str(expBlock)];
...
% accsessed later
BackUp_PracTrial = [expinfo.DataFolder,'Backup\',expinfo.taskName,'GlobalLocal_PracTrials_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];

BackUp_ExpInfo   = [expinfo.DataFolder,'Backup\',expinfo.taskName,'GlobalLocal_ExpInfo_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];
if expinfo.showPractice == 1
