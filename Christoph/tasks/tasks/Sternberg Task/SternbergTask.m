...
%% Trial run, when there is only one block
%Show Shifting Trials

 expBlock = 1;
 ExpTrials = MakeTrial(expinfo, isPractice);
...
%      if expTrial == (round(1/2*expinfo.nExpTrials)+1)
...
%          dImageWait(expinfo,ima);
         ...
         BackUp_Trial     = [expinfo.DataFolder,'Backup\',expinfo.taskName,'Sternberg_Trials_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session),'_Block',num2str(expBlock)];
...
%% Trial presentation, when there is more than one block
...
% accsessed later
BackUp_PracTrial = [expinfo.DataFolder,'Backup\',expinfo.taskName,'Sternberg_PracTrials_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];
BackUp_ExpInfo   = [expinfo.DataFolder,'Backup\',expinfo.taskName,'Sternberg_ExpInfo_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];
if expinfo.showPractice == 1
