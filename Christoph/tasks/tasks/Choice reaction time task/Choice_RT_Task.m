...
ExpTrials = MakeTrial(expinfo, isPractice, PracBlock, expBlock); % function that sets up the trial
 if expBlock > 1
     Pause2=[expinfo.InstFolder  'Pause2.jpg'];
     ima=imread(Pause2, 'jpg');
     dImageWait(expinfo,ima);
 end
...
%      io64(expinfo.ioObj, expinfo.PortAddress,0);% Stop Writing to Output Port     
 ...
 BackUp_Trial     = [expinfo.DataFolder,'Backup\',expinfo.taskName,'Choice_RT_Task ',num2str(expinfo.subject),'_Ses',num2str(expinfo.session),'_Block',num2str(expBlock)];   
...
% accsessed later
BackUp_PracTrial = [expinfo.DataFolder,'Backup\',expinfo.taskName,'Choice_RT_Task_Prac',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];

BackUp_ExpInfo   = [expinfo.DataFolder,'Backup\',expinfo.taskName,'Choice_RT_Task_Exp',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];
if expinfo.showPractice == 1
