...
%     io64(expinfo.ioObj, expinfo.PortAddress, 0); % Stop Writing to Output Port
    ...
    Trial(expTrial).time_probe = TextCenteredOnPos(expinfo,Trial(expTrial).Word,expinfo.centerX,expinfo.centerY,Trial(expTrial).ProbeStimColor,next_flip,Trial(expTrial).ProbeStimMaker);
...
fprintf (fid, '%3d %1d %3d %12s ', expinfo.subject, Trial(expTrial).Block, Trial(expTrial).TrialNum, Trial(expTrial).TaskDescription);
% Print trial specification into data file
fprintf (fid, '%1d %4s %1d %4s %1d %11s ', Trial(expTrial).WordNum, Trial(expTrial).Word ,Trial(expTrial).ColorNum ,Trial(expTrial).Color ,Trial(expTrial).CongruencyNum, Trial(expTrial).Congruency);
