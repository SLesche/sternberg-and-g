...
%     io64(expinfo.ioObj, expinfo.PortAddress, 0); % Stop Writing to Output Port
    ...
    Trial(expTrial).time_probe = TextCenteredOnPos(expinfo,num2str(Trial(expTrial).ProbeStim),expinfo.centerX,expinfo.centerY,expinfo.Colors.gray,next_flip, Trial(expTrial).ProbeStimMaker);
...
fprintf (fid, '%3d %1d %1d %3d %12s ', expinfo.subject, expinfo.session, Trial(expTrial).Block, Trial(expTrial).TrialNum, Trial(expTrial).TaskDescription);
% Print trial specification into data file
fprintf (fid, '%1d %1d %3s %1d ', Trial(expTrial).Task, Trial(expTrial).NumCondition, Trial(expTrial).Condition, Trial(expTrial).TaskConditions);
% Print trial specification into data file
fprintf (fid, '%1d %1s %1d %1s %2s ', Trial(expTrial).NumLetterLeft, Trial(expTrial).LetterLeft,  Trial(expTrial).NumLetterRight, Trial(expTrial).LetterRight,  Trial(expTrial).ProbeStim);
% Print stimuli and variable timings for ISI & ITI in data dile
fprintf (fid, '%1.4f %1.4f ', Trial(expTrial).FIX, Trial(expTrial).ITI);
