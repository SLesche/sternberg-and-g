...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% At the moment, the task is programmed that only the NI condition will be executed. 
% However, the PI condition can also be generated in the background. This 
% depends on the settings of expinfo.Task variable in the expsettings function ( 0 = Physical Identical, 1 = Name Identical). 
% The mask of the experiment and the instructions have to be adapted if you
% selcet another task.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     io64(expinfo.ioObj, expinfo.PortAddress,0);% Stop Writing to Output Port
    ...
    if expBlock < expinfo.Block && expBlock ~= expinfo.Block
     ...
     if expinfo.Task == 0
         BackUp_Trial     = [expinfo.DataFolder,'Backup\',expinfo.taskName,'PosnerPI_Trials_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session),'_Block',num2str(expBlock)];
         save(BackUp_Trial,'ExpTrials');  
     else
         BackUp_Trial     = [expinfo.DataFolder,'Backup\',expinfo.taskName,'PosnerNI_Trials_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session),'_Block',num2str(expBlock)];
         save(BackUp_Trial,'ExpTrials');
     end
 end
...
% accsessed later
if expinfo.Task == 0
BackUp_PracTrial = [expinfo.DataFolder,'Backup\',expinfo.taskName,'PosnerPI_PracTrials_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];
BackUp_ExpInfo   = [expinfo.DataFolder,'Backup\',expinfo.taskName,'PosnerPI_ExpInfo_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];
else
BackUp_PracTrial = [expinfo.DataFolder,'Backup\',expinfo.taskName,'PosnerNI_PracTrials_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];
BackUp_ExpInfo   = [expinfo.DataFolder,'Backup\',expinfo.taskName,'PosnerNI_ExpInfo_S',num2str(expinfo.subject),'_Ses',num2str(expinfo.session)];
end
