...
if expTrial == 1
    ...
    Trial(expTrial).time_fixation = TextCenteredOnPos(expinfo,'+',expinfo.centerX,expinfo.centerY, Trial(expTrial).FixColor,[],Trial(expTrial).FixMarker);
...
%    io64(expinfo.ioObj, expinfo.PortAddress, 0); % Stop Writing to Output Port
 ...
 %   io64(expinfo.ioObj, expinfo.PortAddress, 0); % Stop Writing to Output Port
    ...
    Trial(expTrial).time_probe = TextCenteredOnPos(expinfo,num2str(Trial(expTrial).ProbeStim),expinfo.centerX,expinfo.centerY,Trial(expTrial).ProbeStimColor,next_flip, Trial(expTrial).ProbeStimMaker);
...
fprintf (fid, '%1d %1d %2s %1d %1s ', Trial(expTrial).Task, Trial(expTrial).Shifting,  Trial(expTrial).ProbeStim, Trial(expTrial).Numbers, Trial(expTrial).Letters);
